 ...
 *
 * 功能（对照题目）：
 *   - 6 行列表：Cursor Sens / Cursor Size / Brightness / Volume /
 *     Screen Off / Set Clock（校时入口，复用开机校时界面）
 *   - 交互：光标上下移选行（蓝底高亮）；每行右侧有 [-] [+] 两个按钮，
...
#define SET_ROW_H   30               /* 行高（6 行 → 52..232） */
#define SET_ROWS    6
...
/* 画一行：hover=1 蓝底白字，0 白底黑字。名称 + 右侧当前值 + [-][+] 按钮
...
static void draw_row(uint8_t row, uint8_t hover)
{
    ...
    case 4: name = "Screen Off";
        ...
        break;
    default: name = "Set Clock"; strcpy(val, "Enter >"); break;
    }

    atk_md0280_fill(0, y - 2, SCR_W - 1, y + SET_ROW_H - 3, hover ? ATK_MD0280_BLUE : ATK_MD0280_WHITE);
